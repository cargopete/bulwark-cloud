...
class AuditRunner:
    ...
    def _render_config(self) -> None:
        ...
        core_items = ", ".join(f'"{c}"' for c in self.env.target_core_contracts)
        # `model` is a root-level key, so it MUST precede the [target] table —
        ...
        config = textwrap.dedent(f"""\
            ...
            core_contracts = [{core_items}]
        """)
        # Optional pre-build command (e.g. monorepo dependency install) so
        # `forge build` can resolve remappings into node_modules. Stays inside
        # the [target] table.
        if self.env.pre_build_cmd:
            escaped = self.env.pre_build_cmd.replace('"', '\\"')
            config += f'pre_build_cmd = "{escaped}"\n'
